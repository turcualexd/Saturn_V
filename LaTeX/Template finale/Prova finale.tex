...
\hline
$T_c [K]$ & $P_c [bar]$ & $P_{out} [bar]$ & $t_{p} [ms]$ & O/F & $\dot{m_{fuel}} [kg/s]$ & $\dot{m_{ox}} [kg/s]$ \\
\hline
...
\vspace*{5mm}
Una stima quantitativa per il volume totale necessario alla camera di combsutione per garantire le richieste della stessa è basato su un tempo di permanenza ricavato nel caso dei GG per ogni coppia di propellente. Nel caso dell' F1 si ha:
\vspace*{5mm}
\begin{empheq}{gather*}
	V_{cc} = t_{p}\frac{\dot{m_{gg}}}{\rho_{gc}} = 5\cdot 10^{-3} \left( \frac{53.52 + 22.23}{18.3406} \right) = 
\\
= 0.02065 m^{3} \\	
\end{empheq}
