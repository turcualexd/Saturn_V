...
f_aw  = 0.93;        % /            Turbolent boundary layer recovery factor in gola [intervallo 0.9-0.98] 
R_d = 1125;          % in^2*s*F/btu Thermal resistance caused by the solid deposition in gola

R = (27.38+6.97)/2;  % in           Radius of curvature of nozzle contour (parte nozzle) (utilizzo R1 e Rn di rpa)
...
mu_gg= 46.6e-10 * mm^0.5 * T_c^0.6;     % viscosità gas combusti
Pr = 4*g / ( 9*g -5 );                  % considero costante 

hg = (0.026/((R_t*2)^0.2)) * (mu_gg^0.2 * cp / (Pr^0.6)) * ...
    (p_c * gg / c_star)^0.8 * (((R_t*2) / R)^0.1 ) * ...
    (rapp_A)^0.9 * sigma;               % btu/in^2*sec*F     gas side heat transfer coefficient 
...
v = 0.35;            % /                    Coefficiente di Poisson
...
% Nusselt = C1*Re^0.8*  Pr* 0.4 * rapp_mu ^ 0.14
...
diametro_interno = d_int * 2.54;                      % cm

V_co = (w_f/ro) / (Numero_tubi/2 * (pi*d_int^2)/4);        % in/s   coolant velocity

% calore scambiato nel regenerative per unità di superficie all'altezza
% della gola in funzione del coefficiente H

H = 1/ (1/h_gc + thickness/k_lega + 1/h_c);
q_1 = H *(T_aw-T_co);             % [Btu/in^2*sec]
