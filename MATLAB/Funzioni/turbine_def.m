...
c_p       = 2742.2380 ;     % c_p gas generator from interpolation (J/kgK)
T_gg      = 1062;           % K
T_turb_e  = 888.38;         % K
p_in      = 63.43;          % bar
eps       = 16.4;
p_e       = p_in/eps;
o_f       = 0.416;
g         = 1.128179;       % gamma gas combusti
R         = 294.4417;       % J/kgK
mdot      = 77.92;          % kg/s
omega     = 574.7;          % 1/s
U_over_c0 = 0.225;

% Ipotesi sui rendimenti (AIAA)
eta_ovrll = 0.6;    % rendimento totale turbina (termodinamico + meccanico)
...
% risolvendo il sistema troviamo alfa2 e c2
...
beta_2      = pi - acos(V_2_vec(1) / norm(V_2_vec));
...
beta_3      = acos(V_3_vec(1) / norm(V_3_vec));
...
beta_4      = pi - acos(V_4_vec(1) / norm(V_4_vec));
...
yline(C_1_vec(2), 'LineStyle', '--', color="k")
...
yline(C_1_vec(2) + C_2_vec(2), 'LineStyle', '--', color="k")
...
yline(C_1_vec(2) + C_2_vec(2) + C_3_vec(2), 'LineStyle', '--', color="k")
